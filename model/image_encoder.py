    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i in req_list:
                ...
                feat = self.neck[neck_idx](feat)
                features_list.append(feat)
                neck_idx += 1

        if len(features_list) != len(req_list):
            raise RuntimeError(f"[image_encoder] expected {len(req_list)} features, got {len(features_list)}")
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        if require_adapter:
            self.adapter = MBA(
                dim=dim, red=dim // 3, lora_rank=16, lora_alpha=1.0, use_freq=True, use_ssm=True
            )
        self.attn = Attention(
            dim,
            num_heads=num_heads,
