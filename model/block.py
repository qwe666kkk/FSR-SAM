...
class DetailEnhancement(nn.Module):
    def __init__(self, img_dim, feature_dim, norm, act):
        ...
        self.fusion_conv = nn.Sequential(
            nn.Conv2d(feature_dim * 2 + img_dim, 32, 3, padding = 1, bias = False),
            norm(32),
            act(),
        )
    ...
    def forward(self, img, feature, b_feature, loc_prior=None):
        # Eq. (FGPM gate): the same foreground-confidence map gates the
        # frequency-refined decoder feature and the mapped deep feature.
        if loc_prior is not None:
            loc_d = F.interpolate(
                loc_prior, size=feature.shape[-2:], mode="bilinear", align_corners=False
            )
            feature = feature * (1.0 + loc_d)
            b_feature = b_feature * (1.0 + loc_d)

        img_feature = self.img_in_conv(img)
        img_feature = self.img_er(img_feature) + img_feature
        if loc_prior is not None:
            loc_0 = F.interpolate(
                loc_prior, size=img_feature.shape[-2:], mode="bilinear", align_corners=False
            )
            img_feature = img_feature * (1.0 + loc_0)

        target_size = img_feature.shape[-2:]
        feature = F.interpolate(feature, size=target_size, mode="bilinear", align_corners=False)
        b_feature = F.interpolate(b_feature, size=target_size, mode="bilinear", align_corners=False)

        # Eq. (FGPM prediction): Concat[F_r_bar, D_f_bar, I_d_bar]
        # followed by one 3x3 prediction projection and one 1x1 output layer.
        out_feature = torch.cat([feature, b_feature, img_feature], dim = 1)
